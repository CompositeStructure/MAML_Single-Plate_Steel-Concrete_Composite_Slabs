# -*- coding: utf-8 -*-
"""
MAML-based Few-Shot Prediction Script.
"""
...
# Configuration
INCLUDE_DOUBLE_PLATE_DATA = 1
FIRST_ORDER = False
USE_SCHEDULER = True
USE_KFOLD = True

RUN_IMPORTANCE_ANALYSIS = True
RUN_SHAP = True
SHAP_NSAMPLES = 100
    ...
    for epoch in range(META_EPOCHS):
        ...
        for task in tasks:
            ...
            for _ in range(NUM_INNER_UPDATES):
                support_pred = functional_call(model, fast_params, (task['support_x'],))
                loss = loss_fn(support_pred, task['support_y'])
                grads = torch.autograd.grad(loss, fast_params.values(), create_graph=create_graph, allow_unused=True)
            ...
            query_pred = functional_call(model, fast_params, (task['query_x'],))
            loss_q = loss_fn(query_pred, task['query_y'])
...
def finetune_and_evaluate(meta_model, X_test, y_test, y_scaler):
    if USE_KFOLD:
        print(f"\n--- Starting Fine-tuning (K-Fold={N_SPLITS}) ---")
        splitter = KFold(n_splits=N_SPLITS, shuffle=True, random_state=RANDOM_SEED)
        split_iter = splitter.split(X_test)
        total_folds = N_SPLITS
    else:
        print(f"\n--- Starting Fine-tuning (Single Split: 80% Train, 20% Test) ---")
        indices = np.arange(len(X_test))
        np.random.shuffle(indices)
        split_point = int(0.8 * len(X_test))
    ...
    for fold, (train_idx, test_idx) in enumerate(split_iter):
        print(f"--- Processing Fold {fold + 1}/{total_folds} ---")
        ...
        for epoch in range(FINETUNE_EPOCHS):
            model.train()
            opt.zero_grad()
            train_loss_scaled = loss_fn(model(X_supp), y_supp)
            train_loss_scaled.backward()
            opt.step()

            model.eval()
            with torch.no_grad():
                preds_scaled = model(X_query)
                preds_real = y_scaler.inverse_transform(preds_scaled.cpu().numpy())
                true_real = y_scaler.inverse_transform(y_query.cpu().numpy())
                v_rmse_real = np.sqrt(np.mean((preds_real - true_real) ** 2))
                v_r2 = r2_score(true_real, preds_real)
                val_loss_log.append(v_rmse_real)
                val_r2_log.append(v_r2)

            if (epoch + 1) % 100 == 0:
                print(f"    Fold {fold + 1} Epoch {epoch + 1} | Val RMSE (Real): {v_rmse_real:.2f} | R2: {v_r2:.4f}")

        with torch.no_grad():
            final_preds_scaled = model(X_query)

        preds_un = y_scaler.inverse_transform(final_preds_scaled.numpy())
...
def analyze_feature_importance(meta_trained_model, X_meta_test, y_meta_test, file_prefix="", y_scaler=None):
    ...
    try:
        ...
        path_rank = os.path.join(OUTPUT_DIR, f'{file_prefix}SHAP_Global_importance.xlsx')
...
if __name__ == '__main__':
    ...
    if USE_DYNAMIC_FILENAME:
        ...
        kf_str = "KFold" if USE_KFOLD else "SingleSplit"
        file_prefix = f"{db_str}_{kf_str}_"
