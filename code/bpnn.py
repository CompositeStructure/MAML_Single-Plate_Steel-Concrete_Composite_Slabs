# -*- coding: utf-8 -*-
"""
Script Function: BPNN Prediction with K-Fold CV and SHAP Analysis.
"""
...
def train_and_evaluate_model(X_train, y_train, X_val, y_val, input_dim, scaler_y):
    ...
    for epoch in range(EPOCHS):
        # --- Train Step ---
        model.train()
        train_preds_scaled = model(X_train)
        loss_scaled = loss_fn(train_preds_scaled, y_train)
        optimizer.zero_grad()
        ...
        model.eval()
        with torch.no_grad():
            # 1. Training metrics (in Real Scale)
            tr_preds_real = scaler_y.inverse_transform(train_preds_scaled.detach().cpu().numpy())
            y_tr_real = scaler_y.inverse_transform(y_train.cpu().numpy())

            tr_rmse = np.sqrt(np.mean((tr_preds_real - y_tr_real) ** 2))
            train_loss_log.append(tr_rmse)
            try:
                t_r2 = r2_score(y_tr_real, tr_preds_real)
            except:
                t_r2 = 0
            train_r2_log.append(t_r2)

            # 2. Validation metrics (in Real Scale)
            val_preds_scaled = model(X_val)
            val_preds_real = scaler_y.inverse_transform(val_preds_scaled.cpu().numpy())
            y_val_real = scaler_y.inverse_transform(y_val.cpu().numpy())

            v_rmse_real = np.sqrt(np.mean((val_preds_real - y_val_real) ** 2))
            val_loss_log.append(v_rmse_real)

            try:
                v_r2 = r2_score(y_val_real, val_preds_real)
            except:
                v_r2 = 0
            val_r2_log.append(v_r2)

        if (epoch + 1) % 100 == 0:
            print(f"    Epoch {epoch + 1}/{EPOCHS} | Val RMSE: {v_rmse_real:.2f} kN | Val R2: {v_r2:.4f}")
...
def combine_shap_fold_outputs(output_dir):
    """Combines SHAP results from multiple K-Folds into a single file."""
    ...
    # Save Combined
    cols_order = ['Fold', 'sample_idx', 'feature', 'feature_idx', 'feature_value', 'feature_value_norm',
                  'SHAP value', 'prediction', 'true', 'mean_abs_shap', 'rank', 'Y_idx', 'Y_jitter']
    ...
    df_all[final_cols].to_excel(path_long, index=False)
    ...
    if USE_K_FOLD:
        ...
        for fold, (train_idx, val_idx) in enumerate(kf.split(X_np)):
            ...
            t_loss, t_r2, v_loss, v_r2, preds, y_true, model = train_and_evaluate_model(
                ...
                len(features_list), scaler_y
            )

            results.append({'Fold': fold + 1, 'Val_RMSE': v_loss[-1], 'Val_R2': v_r2[-1]})
            all_preds.append(pd.DataFrame({'Fold': fold + 1, 'True': y_true.flatten(), 'Pred': preds.flatten()}))

            # Save logs
            pd.DataFrame({'epoch': range(1, EPOCHS + 1), 'loss_rmse': v_loss, 'r2': v_r2}).to_excel(
        ...
        pd.DataFrame({'epoch': range(1, EPOCHS + 1), 'val_rmse': v_loss, 'val_r2': v_r2}).to_excel(
